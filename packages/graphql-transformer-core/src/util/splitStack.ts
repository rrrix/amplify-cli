...
export default function splitStack(opts: SplitStackOptions): NestedStacks {
  ...
  function mapMappingToStack(template: Template): { [key: string]: string } {
  ...
  function makeImportValueForGetAtt(resourceId: string, attribute: string): any {
    return Fn.ImportValue(Fn.Join(':', [importExportPrefix, 'GetAtt', resourceId, attribute]));
  }
  ...
  function makeOutputForGetAtt(resourceId: string, attribute: string): Output {
    return {
      ...
      Export: {
        Name: Fn.Join(':', [importExportPrefix, 'GetAtt', resourceId, attribute]),
      },
    };
  }
  ...
  function makeOutputForRef(resourceId: string): Output {
    return {
      ...
      Export: {
        Name: Fn.Join(':', [importExportPrefix, 'Ref', resourceId]),
      },
    };
  }
